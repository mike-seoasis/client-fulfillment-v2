...
function OutlineEditor({
  ...
  content: { outline_json: any; outline_status: string | null; [key: string]: any };
  projectId: string;
  pageId: string;
  ...
  // Re-sync local state from server when not dirty (e.g. after save or external update)
  useEffect(() => {
  ...
  // Move section up/down in page_progression and section_details
  ...
  // Update section headline in page_progression
  ...
  // Remove key point from a section
                ...
                <div key={index} className="bg-white rounded-sm border border-sand-500 p-5">
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => updateSectionDetail(index, 'headline', e.target.value)}
                      className="w-full px-2.5 py-1.5 text-sm bg-sand-50 border border-sand-300 rounded-sm text-warm-900 focus:outline-none focus:ring-2 focus:ring-palm-400/30 focus:border-palm-400 transition-all"
                    />
                  </div>
                  ...
                  <div className="mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => updateSectionDetail(index, 'purpose', e.target.value)}
                      className="w-full px-2.5 py-1.5 text-sm bg-sand-50 border border-sand-300 rounded-sm text-warm-900 focus:outline-none focus:ring-2 focus:ring-palm-400/30 focus:border-palm-400 transition-all"
                    />
                  </div>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="space-y-1.5">
                      {(detail.key_points ?? []).map((point, ptIdx) => (
                        <div key={ptIdx} className="flex items-center gap-2">
                          ...
                          <input
                            type="text"
                            value={point}
                            onChange={(e) => updateKeyPoint(index, ptIdx, e.target.value)}
                            className="flex-1 px-2.5 py-1.5 text-sm bg-sand-50 border border-sand-300 rounded-sm text-warm-900 focus:outline-none focus:ring-2 focus:ring-palm-400/30 focus:border-palm-400 transition-all"
                          />
                          <button
                            type="button"
                            onClick={() => removeKeyPoint(index, ptIdx)}
                            className="p-1 text-warm-400 hover:text-coral-500 transition-colors"
                            title="Remove"
                          >
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        className="text-xs text-lagoon-600 hover:text-lagoon-700 font-medium mt-1"
                      >
                        + Add point
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      value={detail.client_notes ?? ''}
                      onChange={(e) => updateSectionDetail(index, 'client_notes', e.target.value)}
                      rows={2}
                      className="w-full px-2.5 py-2 text-sm bg-amber-50 border border-amber-200 rounded-sm text-warm-900 placeholder-warm-400 focus:outline-none focus:ring-2 focus:ring-palm-400/30 focus:border-palm-400 transition-all resize-none"
                ...
                {lsiTerms.map((item, i) => (
                  <div key={i} className="flex items-center justify-between text-xs">
                    ...
                    {item.target_count != null && (
                      <span className="font-mono text-warm-400">target: {item.target_count}</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-1">
                {variations.map((item, i) => (
                  <div key={i} className="flex items-center justify-between text-xs">
                    ...
                    {item.verbatim_required && (
                      <span className="text-xs text-palm-600 font-medium">verbatim</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {paa.length > 0 && (
            <div className="bg-white rounded-sm border border-sand-500 p-4">
              ...
              <ul className="space-y-1.5">
                {paa.map((q, i) => (
                  <li key={i} className="text-xs text-warm-600 leading-relaxed">{q}</li>
                ))}
              </ul>
            </div>
          )}
                ...
                {competitors.map((comp, i) => (
                  <div key={i} className="text-xs">
                    ...
                    <div className="flex items-center gap-2 text-warm-400">
                      ...
                      {comp.word_count != null && (
                        <span className="font-mono shrink-0">{comp.word_count} words</span>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="max-w-[1600px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-warm-400">
            {saveStatus === 'saving' ? (
              ...
              <>
                ...
                <span className="text-palm-600">Saved</span>
              </>
            ) : saveStatus === 'failed' ? (
              <>
                <div className="w-1.5 h-1.5 rounded-full bg-coral-400" />
                <span className="text-coral-600">Save failed</span>
              </>
            ) : isDirty ? (
              <>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={updateOutlineMutation.isPending || !isDirty}
              className="px-4 py-2 text-sm font-medium text-white bg-palm-500 hover:bg-palm-600 rounded-sm transition-colors disabled:opacity-50"
            >
              {updateOutlineMutation.isPending ? 'Saving...' : 'Save Draft'}
            </button>

            {/* Export to Google Doc (disabled) */}
            <button
              type="button"
              disabled
              className="px-4 py-2 text-sm font-medium text-warm-600 bg-sand-200 rounded-sm opacity-50 cursor-not-allowed"
              title="Coming soon"
            >
              Export to Google Doc
            </button>

            {/* Approve Outline (only in draft) */}
            {isDraft && (
              <button
                type="button"
