...
export default function ClusterContentGenerationPage() {
  ...
  // Derive states from cluster-filtered pages
  const isGenerating = contentGen.isGenerating;
  const isIdle = !isGenerating && clusterPagesCompleted === 0 && clusterPagesFailed === 0;
  ...
  // Summary counts (from cluster-filtered pages)
  const completedPages = clusterPages.filter((p) => p.status === 'complete');
  const failedPages = clusterPages.filter((p) => p.status === 'failed');
      ...
      <div className="bg-white rounded-sm border border-cream-500 p-6 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-warm-gray-900">
            {isGenerating
              ? `Generating content for ${clusterPagesTotal} pages...`
              : isComplete
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between text-sm text-warm-gray-600 mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span>
                  {clusterPagesCompleted} of {clusterPagesTotal} complete
                </span>
              </div>
              <span className="font-medium text-lagoon-600">{clusterProgress}%</span>
            </div>
        ...
        {(isComplete || isFailed) && !isGenerating && !isLinkPlanning && completedPages.length > 0 && (
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              {(() => {
                const eligibleCount = clusterPages.filter(
