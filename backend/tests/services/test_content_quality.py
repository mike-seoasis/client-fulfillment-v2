...
# ---------------------------------------------------------------------------
# Backward compatibility: content_brief kwarg
# ---------------------------------------------------------------------------


class TestContentBriefBackwardCompat:
    """Verify both main functions accept the new content_brief kwarg."""

    def test_run_quality_checks_without_brief(self) -> None:
        """run_quality_checks works without content_brief (backward compat)."""
        pc = _make_page_content(
            page_title="Test",
            bottom_description="Simple content.",
        )
    ...
    def test_run_quality_checks_with_brief(self) -> None:
        """run_quality_checks accepts content_brief kwarg."""
        pc = _make_page_content(
            page_title="Test",
            bottom_description="Simple content.",
        )
        result = run_quality_checks(pc, {}, content_brief={"lsi_terms": []})
        assert isinstance(result, QualityResult)

    def test_run_blog_quality_checks_without_brief(self) -> None:
        """run_blog_quality_checks works without content_brief."""
        fields = {"content": "Simple blog content."}
        result = run_blog_quality_checks(fields, {})
        assert isinstance(result, QualityResult)

    def test_run_blog_quality_checks_with_brief(self) -> None:
        """run_blog_quality_checks accepts content_brief kwarg."""
        fields = {"content": "Simple blog content."}
        result = run_blog_quality_checks(fields, {}, content_brief={"lsi_terms": []})
        assert isinstance(result, QualityResult)
